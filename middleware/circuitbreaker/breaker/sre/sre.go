...
import (
	...
	"github.com/devexps/go-micro/v2/internal/window"
...
var (
	_ breaker.CircuitBreaker = (*Breaker)(nil)
)
...
func NewBreaker(opts ...Option) breaker.CircuitBreaker {
	opt := options{
		success: 0.6,
		request: 100,
		bucket:  10,
...
func (b *Breaker) MarkFailed() {
	// when client reject request locally, continue to add counter let the drop ratio higher.
	b.stat.Add(0)
}
