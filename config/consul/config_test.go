...
const testPath = "go-micro/test/config"

const testKey = "go-micro/test/config/key"
...
func Test_source_Load(t *testing.T) {
	...
	source, err := New(client, WithContext(context.TODO()), WithPath(testPath))
	if err != nil {
		t.Fatal(err)
	}
