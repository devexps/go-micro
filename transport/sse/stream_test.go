...
func TestStreamAddSubscriber(t *testing.T) {
	s := newStream("test-add", 1024, true, false, nil, nil)
	s.run()
	defer s.close()

	assert.Equal(t, StreamID("test-add"), s.StreamID())
...
func TestStreamRemoveSubscriber(t *testing.T) {
	s := newStream("test-remove", 1024, true, false, nil, nil)
	s.run()
	defer s.close()
...
func TestStreamSubscriberClose(t *testing.T) {
	s := newStream("test-close", 1024, true, false, nil, nil)
	s.run()
	defer s.close()
...
func TestStreamDisableAutoReplay(t *testing.T) {
	s := newStream("test-disable-auto-replay", 1024, true, false, nil, nil)
	s.run()
	defer s.close()
...
func TestStreamMultipleSubscribers(t *testing.T) {
	...
	s := newStream("test-multiple", 1024, true, false, nil, nil)
	s.run()
